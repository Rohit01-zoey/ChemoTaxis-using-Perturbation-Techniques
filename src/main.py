...
sine = sine_wave.SineWaveLoader(50, 100, amplitude=10.0, frequency=10.0)
...
network.train(cfg, rnn_model, sine_wave_dict, logger=log_file)

log_file.close() #free up the logger file

plt.figure()
plt.plot(sine_wave_dict['test'][0, :, 0], 'r', label = "True")
fsm_model = fsm.FSM(rnn_model)
generated_sequence = fsm_model(sine_wave_dict['test'][:, 0:1, :], time = 100)
#plt.plot(rnn_model.forward(sine_wave_dict['val'])[0, :, 0], 'b', label = "Predicted")
