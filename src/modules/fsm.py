    ...
    def __call__(self, X, time = 10, ):
        ...
        if self.state == 'generate':
            try:
                ...
                raise ValueError('The input data must be 3D of the shape ({}, 1, {}). The shape of the input data is: {}'.format(X.shape[0], X.shape[2], X.shape))
            ...
            raise ValueError('The state of the FSM must be "generate". The current state is: {}'.format(self.state))